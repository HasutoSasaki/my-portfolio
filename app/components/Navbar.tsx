...
export const Navbar = ({ isDarkMode, setIsDarkMode }: NavbarProps) => {
  ...
  useGSAP(() => {
    ...
    // logo animation
    tl.fromTo(logoRef.current, { y: -50, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8 });
  ...
  return (
    <>
      ...
      >
        ...
        <ul
          ref={menuRef}
          className={`hidden md:flex items-center gap-6 lg:gap-8 rounded-full px-12 py-3 absolute left-1/2 transform -translate-x-1/2
        ...
        <div ref={actionsRef} className="flex items-center gap-4 flex-shrink-0">
          ...
          >
            <Image
              src={isDarkMode ? assets.sunIcon : assets.moonIcon}
              alt="moonIcon"
              className="w-6"
            />
          </button>
          ...
          <button
            className="block md:hidden ml-3 hover:scale-110 transition-transform"
            onClick={openMenu}
          >
            <Image
              src={isDarkMode ? assets.menuWhite : assets.menuBlack}
              alt="menu-button"
              className="w-6"
            />
          </button>
